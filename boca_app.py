...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Arial, sans-serif; }
        ...
        {% if imagem_url %}
        .image-section { height: 35%; background: url('{{ imagem_url }}') no-repeat center center; background-size: cover; display: flex; justify-content: center; align-items: center; color: white; font-size: 18px; font-weight: bold; text-align: center; padding: 20px; border-bottom: 2px solid #333; position: relative; }
        .image-section::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); }
        .image-section div { position: relative; z-index: 1; }
        {% else %}
        .image-placeholder { height: 35%; background: #2c3e50; display: flex; justify-content: center; align-items: center; color: white; font-size: 18px; font-weight: bold; text-align: center; padding: 20px; border-bottom: 2px solid #333; }
        {% endif %}
...
<body>
    <div class="reel-container">
        ...
        <div class="image-section">
            <div>IMAGEM DO WP</div>
        </div>
        {% else %}
        <div class="image-placeholder">
            IMAGEM FORNECIDA PELO WP
        </div>
        {% endif %}
